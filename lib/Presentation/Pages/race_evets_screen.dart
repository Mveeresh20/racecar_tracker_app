...
class _RaceEvetsScreenState extends State<RaceEvetsScreen> {
  ...
  void initState() {
    ...
    _searchController.addListener(_filterEvents); // Listen for search bar changes
  }
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredEvents = _allEvents.where((event) {
          ...
          final raceTypeMatches = event.raceType.toLowerCase().contains(query);
          final trackNameMatches = event.trackName.toLowerCase().contains(query);
  ...
  List<Event> _getSampleEvents() {
    return [
      ...
      Event(
        ...
        currentRacers: 5,
        maxRacers: 10,
        status: EventStatusType.registrationClosed, // Example: this one is closed
        racerImageUrls: [
