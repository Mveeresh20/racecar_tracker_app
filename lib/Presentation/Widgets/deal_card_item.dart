...
class DealCardItem extends StatelessWidget {
  final DealItem deal;
  final DealDetailItem? Function(String) fetchDealDetail;

  
  

  const DealCardItem({Key? key, required this.deal, required this.fetchDealDetail}) : super(key: key);
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              spacing: 8.0,
              runSpacing: 4.0,
              children: deal.parts.map((part) => _buildPartChip(part)).toList(),
            ),
            const SizedBox(height: 12),
            ...
            Row(
              ...
              children: [
                _buildActionButton(
                  ...
                  () {
                    final detail = fetchDealDetail(deal.id);
        if (detail != null) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => DealDetailScreen(deal: detail,dealItem: deal),
            ),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('No deal detail found.')),
          );
        }
